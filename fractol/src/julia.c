...
/*   Updated: 2023/01/14 18:46:54 by mtrautne         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <fractol.h>

//Mandelbrot set: Complex numbers that converge for Z(n+1) = (Z(n))^2 + C.
...
void	ft_julia(t_fract *vars)
{
	...
	while (vars->x < vars->width)
	{
		...
		{
			ft_julia_calc_start_values(vars);
			while (vars->it < vars->max_it && (sqrt(pow(vars->r_z, 2) + \
					pow(vars->i_z, 2)) <= 2))
				ft_julia_calc_z(vars);
...
{
	vars->r_z = ((vars->x / vars->width) * vars->r_range) - \
		(vars->r_range / 2) + vars->r_orgn;
	vars->i_z = ((-1) * (vars->y / vars->height) * vars->i_range) \
